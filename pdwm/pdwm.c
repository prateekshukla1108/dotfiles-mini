...
void shifttags(const Arg *arg);
void shiftwindow(const Arg *arg);
void killallinstances(const Arg *arg);
...
void movetags(const Arg *arg) {
    Client *c = selmon->sel; // Get the currently focused client
    if (!c) return; // Return if there is no focused client

    unsigned int newtags;
    unsigned int curseltags = selmon->tagset[selmon->seltags];
    int i = arg->i;

    // Calculate the new tags based on the shift direction
    if (i > 0) { // Move to the right workspace
        newtags = (curseltags << i) | (curseltags >> (LENGTH(tags) - i));
    } else { // Move to the left workspace
        newtags = curseltags >> (-i) | (curseltags << (LENGTH(tags) + i));
    }

    // Update client's tags and manage its visibility
    c->tags = newtags & TAGMASK; // Ensure tags do not exceed TAGMASK
    focus(NULL); // Clear focus to update the layout
    arrange(selmon); // Arrange windows after tag change
}
...
{
    Client *c = selmon->sel; // Get the currently focused client
    if (!c) return; // Do nothing if no client is selected
    ...
    // Calculate the new tag, wrapping around if necessary
    int new_tag = current_tag + arg->i;
    if (new_tag < 0) new_tag = LENGTH(tags) - 1; // Wrap around to the last tag
    if (new_tag >= LENGTH(tags)) new_tag = 0;    // Wrap around to the first tag

    // Shift the client to the new tag
    c->tags = (1 << new_tag);

    // View the new tag
    Arg view_arg = {.ui = (1 << new_tag)};
    view(&view_arg);
...
void shiftview(const Arg *arg) {
    ...
    // Calculate the next selected tags based on the shift direction
    if (i > 0) // left circular shift
        nextseltags = (curseltags << i) | (curseltags >> (LENGTH(tags) - i));
    else // right circular shift
        nextseltags = curseltags >> (-i) | (curseltags << (LENGTH(tags) + i));

    // Set the argument to the new tagset and switch to it
    a.i = nextseltags;
    view(&a);
}
void shifttag(const Arg *arg)
